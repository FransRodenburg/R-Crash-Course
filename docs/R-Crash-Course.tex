...
## [3] "United States of America" "United Arab Emirates"    
## [5] "Ukraine"                  "Virgin Islands, U.S."    
## [7] "Uganda"                   "United Kingdom"
\end{verbatim}
...
(\ldots) to be included.

\hypertarget{repeated-measures-time-series}{%
\subsection{Repeated Measures \& Time Series}\label{repeated-measures-time-series}}
...
On the github page for the exercises, you can find two (untidy) data files:

\begin{itemize}
\tightlist
\item
  A Google Sheets file that shows \ldots{} categories as separate columns;
\item
  A set of three CSV files that each represent a different replicate of the same experiment.
\end{itemize}

Your goal is to read each of these data sets into R and then convert them to tidy format.

\begin{Shaded}
\begin{Highlighting}[]
\CommentTok{\# (...) to be included}
\end{Highlighting}
\end{Shaded}
...
\tightlist
\item
  Save the first example data to CSV and read it into R, as explained in the \href{https://youtu.be/BGUqZc-Pb8w?t=36}{video}.
\item
  \ldots{}
  X. Create a subset of the data called \texttt{Subset} that contain only measurements of \ldots{}
\end{enumerate}

\hypertarget{verifying-the-results}{%
\section{Verifying the Results}\label{verifying-the-results}}

To see if your data are indeed in tidy format, the script below will attempt to perform an analysis on each. If it works, your solution is correct! Copy the code below and see if your data sets are tidy:
